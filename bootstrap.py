...
#         identity — rewriting it textually poisons the lockfile (it would
#         then have an inconsistent shape: members renamed, but resolved
#         hashes still computed against the old name). Instead, we EXCLUDE
#         uv.lock from the text plan and re-run `uv lock` at the end of
#         bootstrap so the file is regenerated cleanly. Necessary because
#         every pre-commit hook runs `uv run --frozen …` and would refuse
#         a stale lockfile with "Missing workspace member <new-name>".
...
def verify_rename(repo_root: Path) -> None:
    ...
    We deliberately exclude this script (its comments reference the
    template by name as historical context) and uv.lock (it still has
    the old workspace-member name at this point — regenerate_lockfile()
    rewrites it cleanly via `uv lock` immediately after this check).
    If anything else still mentions the template, our replacement plan
...
def regenerate_lockfile(repo_root: Path) -> None:
    """Re-run `uv lock` after the rename so uv.lock matches the new project name.

    Why this is necessary even though we already excluded uv.lock from the
    text-replacement plan:
      uv.lock pins workspace members by name in its [manifest].members list
      and embeds a [[package]] entry whose `name = "python-devcontainer"`.
      After we rewrite pyproject.toml to say `myapp`, the lockfile is OUT
      OF SYNC. Any `uv run --frozen ...` invocation (every pre-commit hook
      uses --frozen as the single-source-of-truth contract — see
      .pre-commit-config.yaml) will refuse to run with:
          error: The lockfile at `uv.lock` needs to be updated, but
                 `--frozen` was provided: Missing workspace member `myapp`.

    Why uv lock and NOT uv sync:
      * `uv lock` only touches uv.lock — fast and side-effect-free.
      * `uv sync` would also re-resolve and reinstall the .venv, which
        is appropriate as the user's NEXT step but doubles bootstrap
        runtime and writes ~100MB of bytecode the user might not want
        yet (they may want to delete .venv and re-create with their own
        Python pin first).
      The "Next steps" message at the end already tells the user to run
      `uv sync --all-packages` themselves.

    Why we shell out instead of importing uv as a library:
      uv is a Rust binary distributed as a wheel — there is no Python
      API. The CLI is the contract.

    Failure handling:
      If `uv lock` fails, we DON'T self-delete. The user gets a
      diagnostic and can investigate / re-run. The repo is partially
      renamed at that point, but bootstrap.py is still on disk so they
      can fix any leftover and re-attempt the lock step manually.
    """
    uv = shutil.which("uv")
    if uv is None:
        sys.exit(
            "error: `uv` not found on PATH. bootstrap.py needs uv to "
            "regenerate uv.lock after the rename. Install uv "
    ...
    result = subprocess.run(
        [uv, "lock"],
        cwd=repo_root,
        # Capture so the user sees uv's output prefixed by our context,
        # not interleaved with our other prints.
        capture_output=True,
        text=True,
        check=False,
    )
    if result.returncode != 0:
        ...
        sys.exit(
            f"\nerror: `uv lock` failed (exit {result.returncode}). "
            f"Resolve the issue above, then re-run `uv lock` manually. "
            f"bootstrap.py was NOT self-deleted so you can inspect the "
            f"intermediate state."
        )
    # Quiet success — uv lock prints its own "Resolved N packages" line
    # we want the user to see. Surface it from stdout/stderr.
    if result.stdout:
...
def main() -> None:
    ...
    # Regenerate uv.lock so it agrees with the renamed pyproject.toml.
    # Must happen BEFORE self-delete so a `uv lock` failure leaves the
    # script on disk for retry. See regenerate_lockfile() for the why.
    print("\n--- Lockfile regeneration ---")
    regenerate_lockfile(repo_root)
    ...
    print("  uv sync --all-packages       # install workspace into .venv")
    print("  uv run pre-commit install    # wire up the git hook")
    print("  uv run pytest")
